...
# Do a basic check for feature files
if [ -z "$(find ${MODEL_DIR} -iname "*.engine.trtpkg" -type f)" ]; then
    echo "ERROR: No features in features directory (searching inside ${MODEL_DIR})"
    echo "Use features/install_feature.sh to install features before building container"
    exit 1
fi
