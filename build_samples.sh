...
then
    ...
    then
        ...
        then
            # glfw not avail
            if [ -e $_INSTALLDIR/extras/GLFW/lib64/libglfw3.so ] && ! $_FORCE
            then
                echo "glfw3: already exists"
            else
                mkdir -p $_INSTALLDIR/build/glfw3
                cd $_INSTALLDIR/build/glfw3
                ...
                $CMAKE \
                    -D CMAKE_INSTALL_PREFIX="$_INSTALLDIR/extras/GLFW" \
                ...
                $_CLEANUP && rm -rf $_INSTALLDIR/build/glfw
            fi
        fi
